...
describe("buildQueryResultCard", () => {
  ...
  it("adds an Open interactive chart action when interactiveChartUrl is provided", () => {
    const data: DataAgentResponseData = {
      type: "timeseries",
      title: "Trend",
      seriesLabels: ["Jan", "Feb"],
    ...
    const attachment: any = buildQueryResultCard(data, "trend", {
      interactiveChartUrl: "https://bot.example.com/charts/abc123",
    });
    const actions = attachment.content.actions as any[];
    const open = actions.find((a) => a.type === "Action.OpenUrl");
    expect(open).toBeDefined();
    expect(open.title.toLowerCase()).toContain("interactive");
    expect(open.url).toBe("https://bot.example.com/charts/abc123");
  });

  it("omits the interactive chart action when no url is provided", () => {
    const data: DataAgentResponseData = {
      type: "timeseries",
      title: "Trend",
      seriesLabels: ["Jan", "Feb"],
