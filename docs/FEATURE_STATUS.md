...
**Extension (not implemented):** if the Data Agent returns its *own*
fully-interactive chart (e.g. a hosted/JS visualization needing zoom/drill-down),
rendering that would require a Teams **Stage View / task module** web view. This
depends on the chart format the Data Agent emits and is deferred until that
format is known.
